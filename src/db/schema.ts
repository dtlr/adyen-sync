...
export const uptMgmtInstallationIdSeq = pgSequence('upt_mgmt_installation_id_seq', {
  startWith: '1',
  increment: '1',
  minValue: '1',
  maxValue: '9223372036854775807',
...
export const symDataDataIdSeq = pgSequence('sym_data_data_id_seq', {
  startWith: '1',
  increment: '1',
  minValue: '1',
  maxValue: '9223372036854775807',
...
export const payAssignedPaymentDevice = pgTable(
  'pay_assigned_payment_device',
  {
    ...
    lastUpdateTime: timestamp('last_update_time', { precision: 6, mode: 'string' }).default(
  ...
  (table) => {
    return {
      payAssignedPaymentDevicePkey: primaryKey({
        columns: [table.deviceId, table.businessUnitId],
...
export const payPaymentDevices = pgTable(
  'pay_payment_devices',
  {
  ...
  (table) => {
    return {
      payPaymentDevicesPkey: primaryKey({
        columns: [table.id, table.businessUnitId],
...
export const devDevicePersonalization = pgTable(
  ...
  (table) => {
    return {
      devDevicePersonalizationPkey: primaryKey({
        columns: [
          table.deviceName,
          table.tagAppId,
          table.tagBusinessUnitId,
          table.tagBrand,
          table.tagDeviceId,
          table.tagCountry,
          table.tagState,
